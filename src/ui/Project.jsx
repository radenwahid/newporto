  ...
  return (
    <Container id="project" className="py-24">
      ...
      <motion.div
        ...
        whileInView="visible"
        viewport={{ once: true }}
        className="flex flex-col items-center justify-center mt-10 gap-28"
      >
        {projects.map((project, index) => (
          <motion.div
            key={index}
            variants={itemVariants}
            className={`flex flex-col lg:flex-row gap-6 ${project.reverse ? 'lg:flex-row-reverse' : ''}`}
          >
            {/* Image */}
            <a
              href={project.link}
            ...
            {/* Description */}
            <div className={`w-full lg:w-1/2 flex flex-col gap-6 lg:justify-between ${project.reverse ? 'items-start text-left lg:ml-16' : 'items-end text-right lg:-ml-16'} z-10`}>
              <div>
                <p className="text-sm tracking-wide text-designColor">
                  Featured Project
                </p>
                <h3 className={`text-2xl font-bold ${isDarkMode ? 'text-lightText' : 'text-gray-800'}`}>
                  {project.title}
                </h3>
              </div>
              <p className={`text-sm md:text-base p-2 md:p-6 rounded-md ${isDarkMode ? 'bg-textBg' : 'bg-white shadow-lg'}`}>
                {project.description}
              </p>
              <ul className={`text-xs md:text-sm tracking-wide flex gap-2 md:gap-5 ${project.reverse ? 'justify-start' : 'justify-end'} ${isDarkMode ? 'text-darkText' : 'text-gray-600'}`}>
